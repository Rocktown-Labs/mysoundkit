...
function PublicUserProfilePage() {
  ...
  const followMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast({ description: `You now follow @${username}.`, title: "Following" });
    },
  });
    ...
    return (
      <Card className="mx-auto max-w-xl">
        <CardContent className="space-y-4 p-8 text-center">
          <h1 className="font-bold text-2xl">{profile.displayName} is an artist</h1>
          <p className="text-muted-foreground">Open their artist profile to hear releases and see live events.</p>
          <Button asChild>
            <Link params={{ username: profile.username }} to="/artist/$username">
              Open Artist Profile
            </Link>
          </Button>
        </CardContent>
  ...
  return (
    <Card className="mx-auto max-w-2xl overflow-hidden">
      ...
      <CardContent className="-mt-12 space-y-5 p-8 text-center">
        ...
        {profile.bio ? <p className="mx-auto max-w-lg text-muted-foreground">{profile.bio}</p> : null}
        ...
        {!isOwnProfile ? (
          ...
          <Button asChild variant="outline">
            <Link to="/library/settings"><Headphones className="mr-2 size-4" />My SoundKit Settings</Link>
          </Button>
        )}
      </CardContent>
    </Card>
