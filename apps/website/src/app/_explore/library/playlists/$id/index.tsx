...
import { columns, type PlaylistTrack } from "./-columns"
    ...
    <div className="px-4 md:px-6 lg:px-8 py-4 md:py-6 lg:py-8">
      ...
      <div className="mb-8 flex items-start gap-6">
        ...
        <div className="flex-1">
          <p className="text-sm text-muted-foreground uppercase tracking-wide mb-2">Playlist</p>
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">My Favorites</h1>
      ...
      <DataTable columns={columns} data={playlistTracks} onAddSong={() => setAddSongOpen(true)} />

      <Dialog open={addSongOpen} onOpenChange={setAddSongOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>Add Song to Playlist</DialogTitle>
            <DialogDescription>Search and select songs to add to your playlist</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <Input placeholder="Search songs..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            <ScrollArea className="h-[300px] rounded-md border p-4">
              <div className="space-y-2">
                ...
                {filteredSongs.length === 0 && <p className="text-center text-muted-foreground py-8">No songs found</p>}
              </div>
            </ScrollArea>
          </div>
        </DialogContent>
