...
export const Route = createFileRoute('/dashboard/battles/upcoming')({
              ...
              <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h2 className="text-xl font-bold">vs {battle.opponent}</h2>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant={battle.status === "confirmed" ? "default" : "secondary"}>
                          {battle.status === "confirmed" ? "Confirmed" : "Pending"}
                        </Badge>
                        <span className="text-sm text-muted-foreground">{battle.genre}</span>
                        <span className="text-sm text-muted-foreground">•</span>
                        <span className="text-sm text-muted-foreground">{battle.format}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-2 lg:min-w-[140px]">
                  <Button asChild className="w-full">
                    <Link to={`/dashboard/battles/my-kit?battle=${battle.id}`}>
        ...
        {upcomingBattles.length === 0 && (
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-muted-foreground mb-4">You don't have any scheduled battles yet</p>
              <Button asChild>
                <Link to="/dashboard/battles/find">Find a Battle</Link>
              </Button>
            </CardContent>
          </Card>
        )}
