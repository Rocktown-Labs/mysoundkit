...
function CareerCalendarPage() {
  ...
  const [newTaskCol, setNewTaskCol] = useState<"todo" | "in_progress" | "scheduled" | "released">("todo");
  ...
  const [customTasks, setCustomTasks] = useState<KanbanTask[]>([
    { category: "promo", column: "in_progress", date: "2026-07-28", id: "k-1", title: "Record TikTok & Reels Cover Teaser" },
    { category: "battle", column: "todo", date: "2026-07-30", id: "k-2", title: "Battle 3 Top Artists in Rap Arena" },
    { category: "promo", column: "scheduled", date: "2026-08-02", id: "k-3", title: "Send Post-Battle Summary Email to Fans" },
  ]);
  ...
  const moveTask = (taskId: string, newCol: "todo" | "in_progress" | "scheduled" | "released") => {
    setCustomTasks((current) =>
      current.map((task) => (task.id === taskId ? { ...task, column: newCol } : task))
    );
    toast({
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Schedule releases, manage promotional campaigns, and track artist milestones.
          </p>
        </div>
            ...
            <DialogContent>
              ...
              <form onSubmit={handleCreateTask} className="space-y-4 pt-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={newTaskCol} onValueChange={(val: any) => setNewTaskCol(val)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
      ...
      <Tabs value={activeTab} onValueChange={(val: any) => setActiveTab(val)}>
          ...
          <Card className="border-border/40 bg-card/40">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Click dates or tasks to jump directly to live rooms and project setup.</CardDescription>
              </div>
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
              ...
              <div className="grid grid-cols-7 gap-2">
                {calendarDays.map((cd, index) => {
                  if (!cd.isCurrentMonth) {
                    return <div key={`empty-${index}`} className="h-28 rounded-xl bg-muted/10 border border-transparent" />;
                  }

                  const matchedProjects = projects.filter((p) => p.releaseDate === cd.dateStr);
                  const matchedParties = parties.filter((p) => p.scheduledStartAt.startsWith(cd.dateStr));
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className={`text-xs font-bold ${isToday ? "text-emerald-500" : "text-muted-foreground"}`}>
                          {cd.dayNum}
                        </span>
                        {isToday && (
                          <Badge className="bg-emerald-500 text-[8px] h-4 px-1">Today</Badge>
                        )}
                      </div>
        ...
        <TabsContent value="kanban" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              { col: "todo", color: "border-amber-500/40 bg-amber-500/5 text-amber-400", title: "1. To Do (Backlog)" },
              { col: "in_progress", color: "border-blue-500/40 bg-blue-500/5 text-blue-400", title: "2. In Progress (Promo)" },
              { col: "scheduled", color: "border-purple-500/40 bg-purple-500/5 text-purple-400", title: "3. Scheduled (Queued)" },
              { col: "released", color: "border-emerald-500/40 bg-emerald-500/5 text-emerald-400", title: "4. Released & Live" },
            ].map((column) => {
              const columnTasks = allKanbanTasks.filter((t) => t.column === column.col);

              return (
                <div key={column.col} className="space-y-3">
                  <div className={`p-3 rounded-xl border font-bold text-xs uppercase tracking-wider flex items-center justify-between ${column.color}`}>
                    <span>{column.title}</span>
                    <Badge variant="secondary" className="text-[10px] font-mono">{columnTasks.length}</Badge>
                  </div>

                  <div className="space-y-3 min-h-[350px] p-2 rounded-2xl border border-border/30 bg-muted/10">
                    {columnTasks.map((task) => (
                      <Card key={task.id} className="border-border/40 bg-card/60 shadow-sm hover:border-primary/40 transition-colors">
                        <CardContent className="p-4 space-y-3">
                          ...
                          {task.date && (
                            <p className="text-xs text-muted-foreground flex items-center gap-1">
                              <Clock className="size-3 text-emerald-400" /> {task.date}
                            </p>
                          )}
                          ...
                          <div className="flex items-center justify-between pt-2 border-t border-border/20 text-xs">
                            <Select
                              value={task.column}
                              onValueChange={(val: any) => moveTask(task.id, val)}
                            >
