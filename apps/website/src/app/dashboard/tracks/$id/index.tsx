...
import { Download, Edit, Share2, Play, Music2, Mic, FileAudio, Disc } from "lucide-react"
...
const mockTrack = {
  ...
  isPublic: true,
  files: {
    instrumental: { name: "summer-vibes-instrumental.wav", size: "45.2 MB", uploaded: true },
    verses: [
  ...
  variants: [
    { type: "clean", file: "summer-vibes-clean.wav", size: "42.1 MB" },
    { type: "dirty", file: "summer-vibes-dirty.wav", size: "42.3 MB" },
  ],
  collaborators: [
    { email: "user@soundkit.app", name: "John Doe", role: "Producer", avatar: "/diverse-user-avatars.png" },
    { email: "collab@soundkit.app", name: "Jane Smith", role: "Vocalist", avatar: "/diverse-user-avatars.png" },
  ],
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-4">
            <div className="flex items-start justify-between">
              <div>
                <h1 className="text-3xl font-bold font-[family-name:var(--font-playfair)]">{mockTrack.name}</h1>
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variant={mockTrack.status === "complete" ? "default" : "secondary"}>{mockTrack.status}</Badge>
              {mockTrack.isPublic && <Badge variant="outline">Public</Badge>}
              {mockTrack.price > 0 && <Badge variant="outline">${mockTrack.price}</Badge>}
            </div>
              ...
              <CardContent>
                <div className="flex items-center justify-between p-3 bg-accent/20 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium">{mockTrack.files.instrumental.name}</p>
                      <p className="text-sm text-muted-foreground">{mockTrack.files.instrumental.size}</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                {mockTrack.files.verses.map((verse, index) => (
                  <div key={verse.id} className="flex items-center justify-between p-3 bg-accent/20 rounded-lg">
                    <div className="flex items-center gap-3">
                      <FileAudio className="h-5 w-5 text-muted-foreground" />
                      <div>
                        <p className="font-medium">Verse {index + 1}</p>
                        <p className="text-sm text-muted-foreground">{verse.size}</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" variant="ghost">
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  {mockTrack.variants.map((variant) => (
                    <div key={variant.type} className="flex items-center justify-between p-3 bg-accent/20 rounded-lg">
                      <div className="flex items-center gap-3">
                        <FileAudio className="h-5 w-5 text-muted-foreground" />
                        <div>
                          <p className="font-medium">{variant.type.toUpperCase()}</p>
                          <p className="text-sm text-muted-foreground">{variant.size}</p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <Button size="sm" variant="ghost">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {mockTrack.collaborators.map((collab) => (
                  <div key={collab.email} className="flex items-center justify-between p-3 bg-accent/20 rounded-lg">
                    <div className="flex items-center gap-3">
                      <Avatar>
