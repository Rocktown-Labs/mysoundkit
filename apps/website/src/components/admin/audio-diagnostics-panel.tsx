...
import {
  useAdminSyncLyricsStemsMutation,
  useCreateDiagnosticJobMutation,
  useDiagnosticJobQuery,
  useDiagnosticJobsQuery,
  useDiagnosticTestsQuery,
  useSearchQuery,
...
export function AudioDiagnosticsPanel() {
  ...
  const searchQuery = useSearchQuery({
    ...
    syncStems = useAdminSyncLyricsStemsMutation(),
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Sync stems & transcribe vocals</CardTitle>
          <CardDescription>
            Runs Demucs separation (vocals + instrumental) and Cloudflare
            Workers AI transcription. Backfill runs are quiet — no lyric
            notifications are sent.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-wrap items-center gap-2">
          <Button
            disabled={syncStems.isPending}
            onClick={() =>
              syncStems.mutate(
                selected.length > 0
                  ? { trackIds: selected.map((track) => track.id) }
                  : { limit: 25, missingOnly: true },
                {
                  onError: () => {
                    toast({
                      description: "Could not queue the sync.",
          ...
          >
            {syncStems.isPending
              ? "Queueing…"
              : (selected.length > 0
                ? `Sync ${selected.length} selected track${selected.length === 1 ? "" : "s"}`
                : "Sync missing lyrics/stems (25)")}
          </Button>
          <span className="text-xs text-muted-foreground">
            Select tracks below for specific IDs, or run the missing-only sweep.
          </span>
        </CardContent>
      </Card>
