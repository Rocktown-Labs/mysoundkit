...
- Added in-house stem separation + lyric transcription (#257): Demucs `htdemucs` vocals/instrumental separation in a zero-internet Cloudflare Container (`apps/stem-separator`, `standard-3`) and Cloudflare Workers AI transcription (`@cf/openai/whisper-large-v3-turbo`, single model for all tracks, language auto-detect) with sequential chunked range reads plus CBR-offset word timings and VTT fallback into timed lyric lines. Enrichment auto-queues on every track settle/finalize and project-track upload (no premium gate, `enrichLyrics` now opt-out), backfill and admin sync (`POST /v1/admin/audio-diagnostics/sync-lyrics-stems`) run quiet with no lyric notifications, backfill/admin eligibility is computed from real v2 stem + machine-lyric outputs, the Lyrics tab shows model/stem badges with smart retry (reuses both vocal and instrumental stems; retries restart lyric-less runs), and StemSplit submission/polling/webhook processing was removed (route kept as a deprecated no-op).
...
- Added audio-native embeddings (P4 of #242): Gemini multimodal audio vectors stored alongside text (`model:audio` suffix, no schema change), gated pipeline hook (off by default), weighted text+audio fusion via `?fuse=`, and an admin spike harness (embed up to 5 tracks, rank per probe word) plus audio-vector indexing controls.
...
- Fixed the public Live Battles route to refresh real battle data and render Featured, Live Now, and Upcoming rails before populated genre rails.
- Fixed battle moderation ownership: BattleBot now controls timed stage transitions, active-artist microphone presets, and battle phase announcements in Arena Chat; manual artist promote/demote and remote mute controls are no longer exposed.
